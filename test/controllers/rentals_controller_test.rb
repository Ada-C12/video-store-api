...
describe RentalsController do
  ...
  describe "checkin" do 
    ...
    it "updates customer movies_checked_in" do 
      post checkin_path, params: rental_data
      body = check_response(expected_type: Hash, expected_status: :success)
      new_rental = Rental.find(body["id"])
      customer = @new_rental.customer
    ...
    it "responds with bad request when passed invalid params" do 
      ...
      expect{post checkin_path, params: invalid_rental_data}.wont_change 'Rental.count'
      body = check_response(expected_type: Hash, expected_status: :bad_request)
      expect(body["errors"]).must_equal "invalid customer or movie"
    end 
  end
end
